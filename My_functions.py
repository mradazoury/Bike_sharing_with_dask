...
def num_name(df):
    ...
    df["season"] = df.season.apply(lambda x: season[x]).astype("category")
    weathersit = {1: "Good", 2: "Acceptable", 3: "Bad", 4: "Chaos"}
    df["weathersit"] = df.weathersit.apply(lambda x: weathersit[x]).astype("category")
    return df
...
def isDaylight(row):
    ...
    row["isDaylight"] = (
        1 if (row["hr"] < sun["sunset"].hour and row["hr"] > sun["sunrise"].hour) else 0
    )
...
def mean_per_hour_3weeks(dataset):
    ...
    for i in range(0, len(dataset)):
        a.append(
            dataset[
                (dataset["instant"] >= ((i + 1) + -21))
                & (dataset["instant"] < (i + 1))
                & (dataset["hr"] == dataset["hr"][dataset["instant"] == i + 1])
            ]["cnt"].mean()
        )
...
def Genetic_P(dataset, target):
    ...
    print(
        "Number of features created out of genetic programing: {}".format(
            gp_features.shape
        )
    )
